...
import { FormTAreaField } from "@/components/_ui/molecules/formFields";
import {
  GET_BOOK_BY_ID,
  CREATE_BOOK,
  UPDATE_BOOK,
} from "@/components/apis/book-services";
...
export default function Form({ book_id, ReloadBook }) {
  ...
  const ReloadBookByID = async () => {
    ...
    if (result.data && Object.values(result.data).length > 0) {
      ...
      if (result.data.image && result.data.image.trim() !== "") {
        const imageUrl = `${process.env.NEXT_PUBLIC_BACKEND_URI}${result.data.image}`;
        setImagePreview(imageUrl);
      } else {
        setImagePreview(null); // Set to null if no image
      }
    } else {
      openModal({
        message: (
