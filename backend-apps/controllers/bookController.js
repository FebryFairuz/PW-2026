...
// Get explorer books (Clustering: popular, new and most read)
exports.getExploringBooks = async (req, res) => {
  try {
    const { search, is_free, language, sort_by, order } = req.query;
    const limit = 10;
    ...
    // Fetch new books (by creation date)
    ...
    // Fetch most read books (by views)
    ...
    res.json({
      ...
      data: {
        ...
        new_releases: newBooks,
        most_read: mostReadBooks,
      },
    });
  } catch (error) {
    console.error("Error fetching books:", error);
    res.status(500).json({
      success: false,
      message: "Failed to fetch exploring books",
      error: error.message,
    });
  }
};
